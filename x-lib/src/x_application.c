...
typedef struct {
        ...
        e_mem_t          shared_application_data_area;
} x_epiphany_control_t;
static x_epiphany_control_t x_epiphany_control = { .initialized = 0 };
...
/* Conversion between task IDs and descriptors. 
   ...
   * the descriptor address is within the address range of the task 
       descriptor table. 
   * the descriptor address points to the start of a descriptor in the
       table
   * the descriptor is in use (executable name is non-empty)

   Validation in x_task_id_descriptor:
   * The supplied ID is a valid index into the task descriptor table. 
   * The descriptor is in use (executable name is non-empty)
*/

static x_task_id_t x_task_descriptor_id (x_task_descriptor_t * descriptor)
{
...
static x_task_descriptor_t * x_task_id_descriptor (x_task_id_t id)
{
...
static int x_get_host_core_count ()
{
...
/* Working memory management within the application data structure
   ...
   xawm_allocz       - allocate and clear space from the high end of working memory,
                       returning an offset to that space
   xawm_alloc_string - allocate space from the high end of working memory for the 
                       supplied string, copy it into that space, and return an offset
                       to it. 
*/

static x_memory_offset_t xawm_allocz (size_t size)
{
...
static x_memory_offset_t xawm_alloc_string (const char *string_to_copy)
{
        x_memory_offset_t result = 0;
...
static x_task_descriptor_t * xtdt_next_available_host_task_descriptor ()
{
        x_task_descriptor_t * task_descriptor_table,
                            * search_start,
                            * search_end,
...
static int xtdt_init_task_descriptor (x_task_descriptor_t *descriptor, 
...
/* xc_connect_by_task_id
   ...
   Validation:
     * The sender and receiver tasks must be configured
     * The keys must be unique per task
...
static int xc_validate_task_key (x_task_id_t task_id, int key, 
...
static int xc_add_task_endpoint (x_task_id_t task_id,
                                 xc_task_connection_lookup_t ***connection_lookup_p,
                                 int new_connection_index)
{
        ...
        if (!(*connection_lookup_p)) {
          ...
          if ((*task_slot_p)->elements_used == (*task_slot_p)->elements_allocated) {
            (*task_slot_p)->elements_allocated += XC_CHUNK_SIZE;
            (*task_slot_p) = 
                realloc(*task_slot_p,
...
static x_return_stat_t xc_connect_by_task_id (x_task_id_t sender,   int sender_key,
...
static x_return_stat_t xc_connect_by_workgroup_coords 
...
static x_return_stat_t xc_setup_application_connections ()
{
        x_return_stat_t      result = X_ERROR;
        ...
        if (x_application->connection_list_offset != 0) {
          ...
          for (task_slot = 0; task_slot < num_task_slots; task_slot++) {
            if (xc_task_connection_index[task_slot] &&
              ...
              if (0 == (global_task_descriptors[task_slot].connection_index =
...
{
        int e_result;
        int task_descriptor_table_length, i;
        // TODO: for each e_result, check it!

        // Sanity checking
        if ( x_epiphany_control.initialized ) {
          printf ("The application has already been initialized\n");
          return X_ERROR;
        }
        ...
        e_result = e_init(NULL);
        e_result = e_reset_system();
        e_result = e_get_platform_info(&(x_epiphany_control.platform));
        if ( ( *workgroup_rows > x_epiphany_control.platform.rows ) ||
             ( *workgroup_rows == 0 ) ) {
          *workgroup_rows = x_epiphany_control.platform.rows;
        }
        if ( ( *workgroup_columns > x_epiphany_control.platform.cols ) ||
             ( *workgroup_columns == 0 ) ) {
          *workgroup_columns = x_epiphany_control.platform.cols;
        }
        if ( *host_task_slots == 0 ) {
          *host_task_slots = x_get_host_core_count ();
        }
        // Allocate and initialise the application's shared memory data structures.
        e_result = e_alloc (&(x_epiphany_control.shared_application_data_area),
                            (X_HOST_PROCESS_SHARED_DRAM_BASE + X_LIB_SECTION_OFFSET),
                            sizeof(x_application_t));
        x_application = (x_application_t*) 
                        (x_epiphany_control.shared_application_data_area.base);
        ...
        x_application->available_working_memory_start =
          (void*)(x_application->working_memory) - (void*)(x_application);
        ...
        x_application->task_descriptor_table_offset =
          xawm_allocz (task_descriptor_table_length * sizeof(x_task_descriptor_t));
        // If required, create a task descriptor for the host process
        if ( caller_executable_file_name != NULL ) {
          xtdt_init_task_descriptor (xtdt_next_available_host_task_descriptor(),  
                                     xawm_alloc_string(caller_executable_file_name));
        }
        // Allocate a workgroup of desired size (as modified above)
        e_result = e_open (&(x_epiphany_control.workgroup), 0, 0,
          ...
          for (row = 0; row < x_application->workgroup_rows; row++) {
            for (col = 0; col < x_application->workgroup_columns; col++) {
              ...
              if (descriptor->executable_file_name == 0) {
                ...
                xtdt_init_task_descriptor (descriptor, executable_file_name_offset);  
              }        
            }
          }
          // Create connections
        ...
        if (x_application == NULL) {
          ...
          if (descriptor->executable_file_name == 0) {
            ...
            if (x_is_workgroup_task(task_id)) {  
              ...
              e_result = e_start (&(x_epiphany_control.workgroup),row,column);
              if (e_result == E_ERR) {
            ...
            if (workgroup_members_to_start == workgroup_size) {
              ...
              for (row = 0; row < x_application->workgroup_rows; row++) {
                for (col = 0; col < x_application->workgroup_columns; col++) {
                  ...
                  if ((descriptor->executable_file_name != 0) && 
                      (descriptor->state == X_VIRGIN_TASK)) {
                    e_result = e_start (&(x_epiphany_control.workgroup),row,col);
                    if (e_result == E_ERR) {
...
x_application_state_t x_finalize_application ()
{
        ...
        if (x_application == NULL) {
          ...
          e_result = e_close (&(x_epiphany_control.workgroup));
          e_result = e_free (&(x_epiphany_control.shared_application_data_area));
          e_result = e_finalize();
...
void x_dump_application_internals (unsigned int level_of_detail);
void x_dump_application_task_descriptor_table (unsigned int level_of_detail);
...
/* x_dump_application_internals
   ...
   Perhaps this should go into its own module? 
*/
